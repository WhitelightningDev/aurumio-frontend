...
type PriceCard = {
  ...
  icon: string;
  priceZAR: number;
  change24hPct: number; // e.g. +1.2 or -0.5
};
...
const Home: React.FC = () => {
  ...
  // Simulated live updates: nudge prices slightly and update timestamp
  useEffect(() => {
    const t = setInterval(() => {
      setPrices((prev) =>
        prev.map((p) => {
          const delta = (Math.random() - 0.5) * (p.priceZAR * 0.002); // ±0.2%
  ...
  const goToMarket = (e: React.MouseEvent) => {
    e.preventDefault();
